...
router.get('/health-score', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const seoScore = Math.min(100, Math.round((allKeywords.length > 0 ? 20 : 0) +
            Math.min(30, top10 * 5) +
        ...
        const productScore = Math.min(100, Math.round((allIdeas.filter(i => i.status !== 'draft').length * 10) +
            Math.min(40, doneTasks * 5) +
            Math.round(completionRate * 30) +
            (allIdeas.length > 0 ? 10 : 0)));
        // ─── Weighted Overall Score ───────────────────────────────────────────
        const overall = Math.round((revenueScore * 0.20) +
            (seoScore * 0.20) +
            (contentScore * 0.15) +
            (socialScore * 0.15) +
            (leadsScore * 0.15) +
...
Be specific, direct, actionable. No fluff.`;
        let explanation = '';
        try {
            explanation = await ai.generate(explanationPrompt, 'You are a concise startup advisor. Max 2 sentences.');
        }
        catch { }
        if (!explanation)
            explanation = `Your ${strongest.label.toLowerCase()} is your strongest area. Focus on improving ${weakest.label.toLowerCase()} to unlock the next level of growth.`;
