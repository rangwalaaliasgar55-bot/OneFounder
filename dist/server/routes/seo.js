...
function generateFallbackSchema(type, data) {
    ...
    switch (type) {
        ...
        case 'FAQPage': return { ...base, mainEntity: (data.faqs || []).map((f) => ({ '@type': 'Question', name: f.question, acceptedAnswer: { '@type': 'Answer', text: f.answer } })) };
        ...
        case 'BreadcrumbList': return { ...base, itemListElement: (data.items || []).map((item, i) => ({ '@type': 'ListItem', position: i + 1, name: item.name, item: item.url })) };
        default: return base;
    }
}
    ...
    try {
        ...
        if (!result.pages) {
            result = {
                ...
                pages: variableList.slice(0, 8).map((v, i) => ({
                    variable: v,
                    ...
                    difficulty: ['easy', 'medium', 'medium', 'easy', 'hard', 'easy', 'medium', 'easy'][i % 8],
